...
**Jaringan Komputer** adalah kumpulan perangkat komputasi yang saling terhubung untuk berbagi data dan sumber daya.

### 🌐 Mengapa Jaringan Penting?

- **Sharing Resources**: File, printer, internet
- **Communication**: Email, chat, video call
- **Collaboration**: Cloud storage, real-time editing
- **Centralized Management**: Server pusat

---

## 🔌 Topologi Jaringan

**Topologi** adalah cara perangkat dihubungkan dalam jaringan.
...
- Data dikirim ke semua perangkat, yang dituju mengambilnya
...
**Kekurangan:**
❌ Jika kabel utama putus, semua mati  
❌ Lambat saat banyak traffic  
...
**Karakteristik:**
- Semua perangkat terhubung ke hub/switch pusat
- Data lewat hub dulu baru ke tujuan

**Kelebihan:**
✅ Jika 1 PC mati, lainnya tetap jalan  
✅ Mudah troubleshooting  
✅ Mudah menambah perangkat  
✅ Performa lebih stabil  

**Kekurangan:**
❌ Jika hub mati, semua mati  
❌ Lebih mahal (banyak kabel)  
❌ Butuh hub/switch  
...
**Karakteristik:**
- Setiap perangkat terhubung membentuk cincin
- Data berjalan searah (clockwise/counterclockwise)
...
**Kekurangan:**
❌ Jika 1 link putus, jaringan mati  
❌ Troubleshooting susah  
...
**Kelebihan:**
✅ Sangat reliable (backup path)  
✅ Jika 1 link mati, ada jalur lain  
✅ Security tinggi  

**Kekurangan:**
❌ Sangat mahal (banyak kabel)  
❌ Instalasi rumit  
❌ Maintenance susah  

**Dipakai:** Internet backbone, military network
...
7 layer dari aplikasi sampai fisik:

| Layer | Nama | Fungsi | Contoh |
|-------|------|--------|--------|
| 7 | **Application** | Interface user & aplikasi | HTTP, FTP, SMTP |
| 6 | **Presentation** | Format & enkripsi data | SSL/TLS, JPEG |
| 5 | **Session** | Manage koneksi session | APIs, Sockets |
| 4 | **Transport** | End-to-end delivery | TCP, UDP |
| 3 | **Network** | Routing & IP addressing | IP, Router |
| 2 | **Data Link** | MAC addressing, error detection | Ethernet, Switch |
| 1 | **Physical** | Transmisi bit via kabel/wireless | Kabel, WiFi |

**Mnemonik:** **A**ll **P**eople **S**eem **T**o **N**eed **D**ata **P**rocessing

### Model TCP/IP (Internet Model)

4 layer yang lebih sederhana:

| Layer | Fungsi | Protocol |
|-------|--------|----------|
| **Application** | HTTP, FTP, DNS, SMTP | Layer 5-7 OSI |
| **Transport** | TCP, UDP | Layer 4 OSI |
| **Internet** | IP, ICMP, ARP | Layer 3 OSI |
| **Network Access** | Ethernet, WiFi | Layer 1-2 OSI |

**TCP/IP** adalah model yang dipakai Internet!
...
#### 1. **Simplex**
- Data mengalir satu arah saja
- Contoh: TV broadcast, radio

#### 2. **Half-Duplex**
- Data bisa dua arah, tapi bergantian
- Contoh: Walkie-talkie

#### 3. **Full-Duplex**
- Data bisa dua arah bersamaan
- Contoh: Telepon, video call
...
- Paling umum (kabel LAN)
- Speed: up to 10 Gbps (Cat 7)
- Jarak: max 100 meter

**2. Coaxial Cable**
- Untuk TV cable, internet cable
- Speed: up to 10 Gbps
- Lebih tahan interferensi

**3. Fiber Optic**
- Transmisi dengan cahaya
- Speed: up to 100 Gbps+
- Jarak: puluhan kilometer
- Dipakai: Internet backbone, datacenter

#### **Wireless (Nirkabel)**

**1. WiFi (802.11)**
- 2.4 GHz & 5 GHz
- Range: ~100 meter indoor
- Speed: up to 9.6 Gbps (WiFi 6)

**2. Bluetooth**
- Range: ~10 meter
- Speed: up to 50 Mbps (Bluetooth 5)
- Untuk peripheral devices

**3. Cellular (4G/5G)**
- Range: beberapa kilometer
...
Setiap device punya alamat IP:

**IPv4:**
- Format: `192.168.1.1`
- 32 bit → 4.3 miliar alamat
- Sudah hampir habis!
...
**Cara kerja:**
1. Kamu ketik `www.google.com`
2. Browser tanya DNS server: "IP-nya google apa?"
3. DNS jawab: `142.250.185.46`
4. Browser konek ke IP tersebut
5. Website muncul!
...
- Masuk via download, email, USB

**Contoh:** WannaCry ransomware (2017) - mengunci 200,000+ komputer di 150 negara

#### 2. **Phishing**
- Email/website palsu untuk mencuri data
- Terlihat seperti bank, e-commerce asli

**Contoh:** Fake email "Akun kamu disuspend, klik link ini"

#### 3. **DDoS (Distributed Denial of Service)**
- Banjiri server dengan traffic palsu
- Server down, service unavailable

**Contoh:** Attack ke GitHub (2018) - 1.35 Tbps traffic

#### 4. **Man-in-the-Middle (MITM)**
- Hacker di tengah komunikasi
- Mencuri data yang dikirim

**Skenario:** Kamu konek WiFi publik → hacker lihat semua traffic → password dicuri

#### 5. **SQL Injection**
- Inject code SQL ke form input
- Akses database tanpa izin

\`\`\`sql
-- Normal: SELECT * FROM users WHERE username='admin'
-- Injection: admin' OR '1'='1
-- Result: Login tanpa password!
\`\`\`
...
### 1. **Firewall**
- Filter traffic masuk/keluar
- Block port yang tidak dipakai
- Monitor suspicious activity

### 2. **Enkripsi**
- Ubah data jadi kode rahasia
- Hanya penerima bisa baca

**Contoh:**
- HTTPS (lock icon di browser)
...
- Jangan pakai tanggal lahir/nama

**Two-Factor Authentication (2FA):**
- Password + OTP (one-time code)
- Lebih aman dari password saja

### 5. **VPN (Virtual Private Network)**
- Enkripsi koneksi internet
- IP address tersembunyi
- Bypass geo-restriction

### 6. **Update & Patch**
- Update OS dan software regularly
- Security patch tutup celah keamanan
- Auto-update ON
...
**Komponen:**
- **Modem**: Koneksi ke ISP
- **Router**: Bagi koneksi ke device
- **Switch**: Tambah port LAN
- **Access Point**: Extend WiFi

### Tips Keamanan WiFi:

✅ Ganti password default router  
✅ Gunakan WPA3 encryption (bukan WEP)  
✅ Hidden SSID (opsional)  
...
\`\`\`bash
ping google.com
# Result: Latency, packet loss

traceroute google.com
# Result: Semua hop dari komputer ke google
\`\`\`
...
**Jaringan adalah tulang punggung era digital.**

**Key Points:**
- Topologi star paling umum dipakai
- TCP/IP adalah protokol internet
- Security adalah prioritas utama
- Update dan enkripsi selalu!

**Best Practices:**
1. Gunakan password manager
2. Enable 2FA semua akun
3. Jangan klik link mencurigakan
4. Update OS dan apps
...
1. Gambar topologi jaringan sekolahmu, jelaskan mengapa topologi itu dipilih
2. Cek IP address komputermu, catat IPv4 dan gateway
3. Ping ke google.com dan youtube.com, bandingkan latency-nya
4. Analisis: Mengapa HTTPS lebih aman dari HTTP?
